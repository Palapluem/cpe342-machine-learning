...
\begin{center}
    ...
    {\large Activity: Estimating Customer Lifetime on Telco Churn Dataset}\\[6pt]
    \textcolor{accentblue}{67070501042 วิศิษฐ์ สุวรรณเนาว์ (WISIT SUWANNAO)}
\end{center}

\noindent\rule{\textwidth}{0.4pt}\vspace{4pt}

\section{Problem Overview \& Objectives (บทนำและภาพรวมของปัญหา)}
...
\section{Theoretical Framework (กรอบทฤษฎีและคณิตศาสตร์ จาก Lecture 4)}

\subsection{ข้อจำกัดของ Regression ทั่วไปกับข้อมูล Time-to-Event}
...
\subsection{นิยามทางคณิตศาสตร์ของฟังก์ชันการอยู่รอดและฟังก์ชันความเสี่ยง}
...
\subsection{ตัวประมาณค่า Kaplan-Meier (Product-Limit Estimator)}
...
เมื่อ:
\begin{itemize}[leftmargin=18pt, itemsep=2pt]
    \item $t_1 < t_2 < \dots < t_k$ คือลำดับเวลาที่เกิดเหตุการณ์ยกเลิกบริการจริง
    \item $d_i$ คือจำนวนลูกค้าที่เกิดเหตุการณ์ยกเลิกบริการ ณ เวลา $t_i$
    \item $n_i$ คือจำนวนลูกค้าที่ยังคงมีความเสี่ยง (Risk Set) ณ จุดเวลาก่อน $t_i$ ($n_i$ รวมทั้งผู้ที่กำลังจะยกเลิกและผู้ที่ถูกเซนเซอร์หลังเวลา $t_i$)
\end{itemize}
...
\subsection{การทดสอบสมมติฐาน Log-Rank Test}
...
\section{Dataset Overview \& Preprocessing (ชุดข้อมูลและการเตรียมตัวแปร)}
...
\section{Experimental Results \& Diagnostic Plots (ผลลัพธ์การวิเคราะห์และกราฟสถิติ)}

\subsection{1. เส้นโค้งการอยู่รอดของประชากรโดยรวม (Overall Survival Curve)}
...
\subsection{2. เปรียบเทียบเส้นโค้งการอยู่รอดตามช่องทางการชำระเงิน (Survival Curves by Payment Method)}
...
\subsection{3. การวิเคราะห์ความเสี่ยงสะสม (Cumulative Hazard Function Analysis)}
...
\subsection{4. การแจกแจงอัตรา Churn และระยะเวลา Tenure}
...
\subsection{5. ตารางสรุปผลลัพธ์เชิงปริมาณ (Quantitative Summary of Results)}
\begin{table}[H]
\centering
...
\subsection{6. ผลการทดสอบทางสถิติ Log-Rank Test}
\begin{table}[H]
\centering
...
\section{In-depth Discussion \& Business Insights (การอภิปรายผลเชิงลึก)}

\subsection{1. การวิเคราะห์กลุ่มความเสี่ยงสูง: Electronic Check}
...
\subsection{2. ประสิทธิภาพของระบบชำระเงินอัตโนมัติ (Automatic Payment Channels)}
...
\subsection{3. พฤติกรรมเฉพาะตัวของกลุ่ม Mailed Check}
...
\section{Strategic Recommendations (ข้อเสนอแนะเชิงกลยุทธ์)}
...
\section{Conclusion (สรุปผลการดำเนินงาน)}
...
\newpage
\appendix
\section{Source Code Implementation (ภาคผนวก: โค้ดโปรแกรมภาษา Python)}

\begin{lstlisting}[style=pycode, caption={โปรแกรมการวิเคราะห์การอยู่รอด Kaplan-Meier และ Log-Rank Test บน Telco Churn Dataset}]
import pandas as pd
import numpy as np
...
# 1. Load data and extract time-to-event variables
df = pd.read_csv('Telco-Churn.csv')
T = df['tenure']
E = (df['Churn'] == 'Yes').astype(int)

# 2. Overall Kaplan-Meier Survival Estimation
kmf_overall = KaplanMeierFitter()
kmf_overall.fit(T, event_observed=E, label='All Customers (N = 7,043)')

# 3. Segmented Survival by Payment Method
...
# 4. Multivariate Log-Rank Test
logrank_res = multivariate_logrank_test(T, df['PaymentMethod'], E)
print(f"Chi-square: {logrank_res.test_statistic:.4f}, p-value: {logrank_res.p_value:.4e}")

# 5. Pairwise Log-Rank Test
pairwise_res = pairwise_logrank_test(T, df['PaymentMethod'], E)
print(pairwise_res.summary)
\end{lstlisting}
