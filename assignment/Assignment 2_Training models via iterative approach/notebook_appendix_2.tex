...
1. \textbf{Learning Curve (Loss over Iterations):} Displays the reduction in MSE Loss from $0.0465 \rightarrow 0.0005$ across iterations, proving smooth asymptotic convergence with learning rate $\eta = 1.0$.
2. \textbf{Parameter Evolution (Trajectory):} Illustrates how parameters $C_0, C_1, C_2$ evolve from $(0.5, 0.5, 0.1)$ toward stable optimal values $(0.9927, 0.9587, -1.3296)$.
3. \textbf{Fitted Model vs Original Data:} Compares the actual observations against the fitted non-linear curve $\hat{y} = 0.9927 + 0.9587 e^{-1.3296 x}$.
4. \textbf{Residual Plot:} Shows that prediction residuals ($y_i - \hat{y}_i$) are randomly distributed around 0 with constant variance, verifying homoscedasticity and absence of systematic bias.
\end{tcolorbox}
\needspace{6\baselineskip}
\vspace{0.8em}
\noindent\textbf{In [5]:}
...
<>:76: SyntaxWarning: invalid escape sequence '\h'
<>:87: SyntaxWarning: invalid escape sequence '\h'
C:\Users\Admin\AppData\Local\Temp\ipykernel_18520\3103374935.py:31: SyntaxWarning: invalid esca ...
  plt.plot(x_line, y_line, 'r-', linewidth=3, label=f'Fitted Model: $\hat{{y}} = {C0:.2f} + {C1 ...
C:\Users\Admin\AppData\Local\Temp\ipykernel_18520\3103374935.py:47: SyntaxWarning: invalid esca ...
  plt.ylabel('Residuals ($y_i - \hat{y}_i$)')
C:\Users\Admin\AppData\Local\Temp\ipykernel_18520\3103374935.py:76: SyntaxWarning: invalid esca ...
  ax3.plot(x_line, y_line, 'r-', linewidth=3, label=f'Fitted Model: $\hat{{y}} = {C0:.2f} + {C1 ...
C:\Users\Admin\AppData\Local\Temp\ipykernel_18520\3103374935.py:87: SyntaxWarning: invalid esca ...
  ax4.set_ylabel('Residuals ($y_i - \hat{y}_i$)')
\end{lstlisting}
\begin{center}
