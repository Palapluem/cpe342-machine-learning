...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.6\textwidth]{nb_out_plot_1.png}
\end{center}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
✨ \textbf{ภาพประกอบที่ 1 (Regression Line)}: แสดงความสัมพันธ์ระหว่างงบโฆษณา (Advertising Budget) และยอดขาย (Sales) พร้อมกับเส้นความสัมพันธ์เชิงเส้น (Regression Line) ที่ได้จากสมการ OLS\\
\end{tcolorbox}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.6\textwidth]{nb_out_plot_2.png}
\end{center}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
✨ \textbf{ภาพประกอบที่ 7 (Actual vs Predicted)}: เปรียบเทียบยอดขายจริง (Actual) กับยอดขายที่แบบจำลองทำนายได้ (Predicted) จุดที่เรียงใกล้เส้นประทแยงมุมแสดงให้เห็นว่าโมเดลทำนายได้ค่อนข้างแม่นยำ\\
\end{tcolorbox}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.6\textwidth]{nb_out_plot_3.png}
\end{center}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
✨ \textbf{ภาพประกอบที่ 2 (Residuals Lollipop Plot)}: แสดงค่าความคลาดเคลื่อน (Residuals) ของข้อมูลแต่ละจุด โดยเปรียบเทียบระยะห่างในแนวดิ่งระหว่างค่าจริงกับค่าที่โมเดลทำนายได้\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [13]:}
\begin{lstlisting}[style=pycode]
...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.6\textwidth]{nb_out_plot_4.png}
\end{center}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
✨ \textbf{ภาพประกอบที่ 3 (Residuals vs Fitted)}: ตรวจสอบรูปแบบความคลาดเคลื่อน (Non-linear patterns) และความแปรปรวนที่คงที่ (Homoscedasticity) หากจุดข้อมูลกระจายตัวแบบสุ่มรอบเส้น 0 ถือว่าเป็นโมเดลที่ดี\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [14]:}
\begin{lstlisting}[style=pycode]
...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.6\textwidth]{nb_out_plot_5.png}
\end{center}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
✨ \textbf{ภาพประกอบที่ 4 (Residuals Distribution)}: กราฟฮิสโตแกรมการแจกแจงความถี่ของค่า Residuals เพื่อตรวจสอบว่ามีการแจกแจงเข้าใกล้การแจกแจงแบบปกติ (Normality) หรือไม่\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [15]:}
\begin{lstlisting}[style=pycode]
...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.6\textwidth]{nb_out_plot_6.png}
\end{center}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
✨ \textbf{ภาพประกอบที่ 5 (Q-Q Plot)}: กราฟเปรียบเทียบ Quantile ของค่า Residuals กับการแจกแจงแบบปกติมาตรฐาน ยิ่งจุดเรียงตัวชิดเส้นทแยงมุมสีแดงมากเท่าใด ยิ่งแสดงถึงความเป็นโค้งปกติมากเท่านั้น\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [16]:}
\begin{lstlisting}[style=pycode]
...
\end{lstlisting}
\begin{center}
\includegraphics[width=0.6\textwidth]{nb_out_plot_7.png}
\end{center}
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
✨ \textbf{ภาพประกอบที่ 6 (Scale-Location)}: ตรวจสอบความเท่าเทียมกันของความแปรปรวนของความคลาดเคลื่อน (Homoscedasticity) ตลอดช่วงค่าที่ทำนายได้ เส้นสีแดงควรมีลักษณะราบขนานกับแกน X\\
\end{tcolorbox}