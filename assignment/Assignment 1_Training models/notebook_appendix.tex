...
\textbf{\Large CPE 342 Machine Learning — Assignment 1: Training Models}\\
\\
...
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
\textbf{\large 0. Imports and Setup}\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [1]:}
...
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
\textbf{\large 2. Task 1: OLS Regression}\\
\\
\textbf{2a. Setting up the Normal Equations}\\
\\
Minimise $S(\alpha, \beta) = \sum\_i (Y\_i - \alpha - \beta X\_i)^2$ gives the Normal Equations:\\
\\
$$\begin{bmatrix} n & \sum X\_i \\ \sum X\_i & \sum X\_i^2 \end{bmatrix} \begin{bmatrix} \alpha \\ \beta \end{bmatrix} = \begin{bmatrix} \sum Y\_i \\ \sum X\_i Y\_i \end{bmatrix}$$\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [3]:}
\begin{lstlisting}[style=pycode]
...
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
\textbf{2b. Solving for Coefficients}\\
\\
**Method 0 — Closed-form via $S\_{xx}$, $S\_{xy}$** (derived directly from Normal Equations):\\
\\
$$\beta = \frac{S\_{xy}}{S\_{xx}}, \qquad \alpha = \bar{Y} - \beta \bar{X}$$\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [4]:}
\begin{lstlisting}[style=pycode]
...
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
\textbf{\large 3. Task 2: Interpretation}\\
\\
\textbf{3a. Meaning of Slope and Intercept}\\
\\
...
\\
\textbf{3b. Prediction at X = 12 (\$12,000 budget)}\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [9]:}
\begin{lstlisting}[style=pycode]
...
\begin{lstlisting}[style=output]
7 individual plots generated successfully using strict Sarabun FontProperties.
\end{lstlisting}
\begin{center}
...
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
\textbf{\large 4. Task 3: Model Evaluation}\\
\\
\textbf{4a. R² Calculation}\\
\\
$$R^2 = 1 - \frac{SS\_{\text{res}}}{SS\_{\text{tot}}}, \quad SS\_{\text{res}} = \sum e\_i^2, \quad SS\_{\text{tot}} = \sum (Y\_i - \bar{Y})^2$$\\
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [11]:}
\begin{lstlisting}[style=pycode]
...
\vspace{1em}
\begin{tcolorbox}[colback=gray!5!white,colframe=gray!50!black,boxrule=0.5pt,arc=2pt,left=2pt,right=2pt,top=2pt,bottom=2pt]
\textbf{\large 6. Summary of Results}\\
\\
| Task | Answer |\\
|------|--------|\\
