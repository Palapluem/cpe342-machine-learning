...
\begin{itemize}
\item \textbf{Slope (β ≈ 1.2667):} For each additional \$1,000 in advertising budget, sales are predicted to increase by approximately \textbf{1,267 units}. This is an association in the sample, not proof of causation.
\item \textbf{Intercept (α ≈ 1.7333):} At zero advertising budget (X = 0), predicted sales ≈ \textbf{1,733 units}. X = 0 is outside the observed range (1–10), so this is an extrapolated baseline.
\end{itemize}
...
\item Residual mean ≈ 0 confirms the model is unbiased over the sample.
\end{itemize}
\end{tcolorbox}
\vspace{1em}
\noindent\textbf{In [12]:}
