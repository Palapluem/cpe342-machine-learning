...
\begin{align*}
    ...
    S_{xy} &= \sum_{i=1}^{n}(X_i - \bar{X})(Y_i - \bar{Y}) = \sum X_iY_i - \frac{\sum X_i \sum Y_i}{n} = 583 - \frac{(55)(87)}{10} = 583 - 478.5 = 104.5
\end{align*}
จะได้:
\[
...
\smallskip
สามารถยืนยันด้วยโค้ด Python:
\begin{minipage}{\textwidth}
\begin{lstlisting}[style=pycode, caption=OLS computation using $S_{xx}$/$S_{xy}$ method]
import numpy as np

X = np.array([3.,5.,2.,7.,8.,1.,4.,6.,9.,10.])
Y = np.array([6.,9.,4.,10.,12.,3.,7.,8.,13.,15.])

X_bar, Y_bar = X.mean(), Y.mean()    # 5.5, 8.7
...
print(f"Fitted: Y_hat = {alpha:.4f} + {beta:.4f}*X")
\end{lstlisting}
\begin{lstlisting}[style=output]
beta  = 1.266667
alpha = 1.733333
Fitted: Y_hat = 1.7333 + 1.2667*X
\end{lstlisting}
\end{minipage}
...
\begin{ansbox}
\textbf{สรุป 1c:} \quad สมการ Fitted Line คือ:
\[
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{scatter_regression.pdf}
    \caption{Scatter Plot แสดงความสัมพันธ์ระหว่างงบโฆษณากับยอดขาย พร้อมเส้น Fitted Line $\hat{Y} = 1.7333 + 1.2667X$}
\end{figure}
\newpage
...
\begin{minipage}{\textwidth}
\begin{lstlisting}[style=pycode, caption=Computing $R^2$ from residuals]
Y_hat     = alpha + beta * X
residuals = Y - Y_hat
SS_res    = np.sum(residuals**2)           # 3.7333
SS_tot    = np.sum((Y - Y_bar)**2)         # 136.1
R2        = 1 - SS_res / SS_tot
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{diagnostics_panel.pdf}
    \caption{Residual Diagnostics Panel — (top-left) Residuals vs Fitted แสดงว่า residuals ไม่มีรูปแบบชัดเจนรอบเส้นศูนย์; (top-right) Residual Distribution การกระจายใกล้เคียง Normal; (bottom-left) Q-Q Plot ยืนยันความเป็น Normal; (bottom-right) Scale-Location ไม่พบ Heteroscedasticity}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{actual_vs_fitted.pdf}
    \caption{Actual vs Fitted Sales — จุดทุกจุดอยู่ใกล้เส้น Perfect Fit ($Y = \hat{Y}$) ด้านแดงพระ แสดงว่าโมเดล fit ข้อมูลได้ดี}
\end{figure}
\newpage
...
\smallskip
\noindent\textit{\small โค้ดสำหรับคำนวณและสร้างกราฟทั้งหมดอยู่ใน \texttt{Assignment\_1\_OLS.ipynb}}
